...
describe('editorStore – road ops actions', () => {
  beforeEach(() => {
  ...
  describe('cloneRoad', () => {
    ...
    it('should push onto undo stack', () => {
      const store = useEditorStore.getState();
      store.addRoad(makeRoad('r1'));
      const undoDepthBefore = useEditorStore.getState().undoStack.length;
      store.cloneRoad('r1', 'r1-clone', [0, 0]);
      expect(useEditorStore.getState().undoStack.length).toBeGreaterThan(undoDepthBefore);
    });
  ...
  describe('reverseRoad', () => {
    ...
    it('should push onto undo stack', () => {
      ...
      store.reverseRoad('r1');
      expect(useEditorStore.getState().undoStack.length).toBeGreaterThan(before);
    });
