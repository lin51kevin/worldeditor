...
describe('StatusBar', () => {
  ...
  it('computes scale bar label dynamically from viewportMpp', () => {
    // mpp = 0.1 → scaleDist = 100 * 0.1 = 10 → label "10m"
    render(<StatusBar />);
    expect(screen.getByText('10m')).toBeInTheDocument();
  });

  it('shows km label when zoomed out', () => {
    // mpp = 20 → scaleDist = 100 * 20 = 2000 → label "2km"
    act(() => {
  ...
  it('shows smaller label when zoomed in', () => {
    // mpp = 0.01 → scaleDist = 100 * 0.01 = 1 → label "1m"
    act(() => {
  ...
  it('scale bar track width is always fixed at 100px', () => {
    // Regardless of mpp, bar width stays constant
    act(() => {
      useProjectStore.setState({ viewportMpp: 5.0 });
    });
    const { container } = render(<StatusBar />);
