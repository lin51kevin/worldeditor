...
export function createPlatformMock(vertices = new Float32Array([1, 2, 3, 0.3, 0.3, 0.3, 1])): PlatformService {
  return {
    ...
    setMinLineWidthM: vi.fn().mockResolvedValue(undefined),
    ...
    generateObjectVerticesCached: vi.fn().mockResolvedValue(new Float32Array()),
    ...
    generateSingleRoadSurfaceVerticesCached: vi.fn().mockResolvedValue(vertices),
    ...
    loadGaussianSplatsNative: vi
      .fn()
      .mockResolvedValue({ meta: { count: 0, shDegree: 0, shStride: 12, layoutVersion: 2, layoutName: 'transform-f32-opacity-sh-f16', origin: [0, 0, 0], min: [0, 0, 0], max: [0, 0, 0] }, buffer: new Uint32Array(0) }),
  };
}
...
export function createRendererMocks() {
  return {
    ...
    setLineWidthScaleCallback: vi.fn(),
