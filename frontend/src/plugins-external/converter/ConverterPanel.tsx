/**
 * External Batch Converter panel — chains a registered importer to an exporter.
 * Trusted first-party plugin: reads importers/exporters from the shared host
 * contribution store (reactive).
 */
...
export default function ConverterPanel() {
  ...
  const importers = useMemo(() => rawImporters.filter((item) => !item.disabled), [rawImporters]);
  const exporters = useMemo(() => rawExporters.filter((item) => !item.disabled), [rawExporters]);
  ...
  useEffect(() => {
    ...
    if (!sourceId && firstImporter) setSourceId(firstImporter.id);
    if (!targetId && firstExporter) setTargetId(firstExporter.id);
  ...
  const convert = async () => {
    if (!sourceImporter || !targetExporter || files.length === 0 || running) return;
