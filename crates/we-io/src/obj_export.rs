...
fn generate_road_mesh(road: &Road, vertices: &mut Vec<[f64; 3]>, faces: &mut Vec<[usize; 3]>) {
    ...
    for i in 0..=steps {
        ...
        vertices.push([x + half_width * perp.cos(), y + half_width * perp.sin(), 0.0]);
        vertices.push([x - half_width * perp.cos(), y - half_width * perp.sin(), 0.0]);
    }
    ...
    match &geom.geo_type {
        ...
        GeometryType::Arc { curvature } => {
            if curvature.abs() < 1e-10 {
                (geom.x + ds * geom.hdg.cos(), geom.y + ds * geom.hdg.sin(), geom.hdg)
            } else {
                let r = 1.0 / curvature;
                ...
                (cx + r * theta.cos(), cy + r * theta.sin(), geom.hdg + dtheta)
            }
        }
        _ => (
            geom.x + ds * geom.hdg.cos(),
...
mod tests {
    ...
    fn test_export_single_road_has_vertices() {
        let project = Project { roads: vec![straight_road("r0", 20.0)], ..Default::default() };
        let obj = export_project_to_obj(&project);
    ...
    fn test_export_vertex_z_is_zero() {
        let project = Project { roads: vec![straight_road("r0", 10.0)], ..Default::default() };
        let obj = export_project_to_obj(&project);
    ...
    fn test_export_multiple_roads_more_vertices() {
        let p1 = Project { roads: vec![straight_road("r0", 20.0)], ..Default::default() };
        let p2 = Project { roads: vec![straight_road("r0", 20.0), straight_road("r1", 30.0)], ..Default::default() };
        let count1 = export_project_to_obj(&p1).lines().filter(|l| l.starts_with("v ")).count();
        let count2 = export_project_to_obj(&p2).lines().filter(|l| l.starts_with("v ")).count();
