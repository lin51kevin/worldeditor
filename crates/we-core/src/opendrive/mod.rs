    ...
    fn test_parking_space_round_trip() {
        ...
        let p = project_with(vec![{
            ...
            r.objects = vec![RoadObject {
                ...
                position: Point3D { x: 1.113, y: 3.616, z: 0.01, id: None },
                orientation: 0.0,
        ...
        assert!(xml.contains(r#"type="ParkingSpace""#), "Type should be ParkingSpace");
        assert!(xml.contains("<outline>"), "Should have <outline> wrapper");
        assert!(xml.contains("cornerLocal"), "Should have cornerLocal elements");
        assert!(xml.contains(r#"hdg="1.5707963249607597""#), "hdg must be preserved, not orientation");
        assert!(xml.contains(r#"orientation="none""#), "orientation should be 'none'");
        assert!(!xml.contains("<roadObjects>"), "Should not use <roadObjects> wrapper");
        assert!(!xml.contains("roadObject"), "Should use <object> not <roadObject>");
    ...
    fn test_parkinglot_xodr_parse() {
        ...
        if let Ok(xodr) = xodr {
            ...
            let parking_count: usize = project
                ...
                .count();
            assert!(parking_count >= 16, "Expected at least 16 parking spaces, got {parking_count}");

            // Verify a specific parking space (id=58) has correct data
            let ps58 = project.roads.iter()
                .flat_map(|r| r.objects.iter())
                .find(|o| o.id == "58");
            ...
            let xml = write_xodr(&project).unwrap();
            let re = parse_xodr(&xml).unwrap();
            let re_parking: usize = re.roads.iter()
                .flat_map(|r| r.objects.iter())
                .filter(|o| o.object_type == ObjectType::ParkingSpace)
                .count();
            assert_eq!(re_parking, parking_count, "Round-trip should preserve parking count");

            // Verify hdg is preserved after round-trip
            let re_ps58 = re.roads.iter()
                .flat_map(|r| r.objects.iter())
                .find(|o| o.id == "58");
            assert!(re_ps58.is_some());
            let re_ps58 = re_ps58.unwrap();
            assert!((re_ps58.hdg - ps58.hdg).abs() < 1e-10, "hdg must survive round-trip");
            assert_eq!(re_ps58.corners.len(), ps58.corners.len(), "corners count must survive round-trip");
        }
    }
    ...
    fn test_parkinglot_all_roads_have_valid_geometry() {
        ...
        for road in &project.roads {
            ...
            println!(
                ...
                road.id, pts.len(), min_x, max_x, min_y, max_y
            );
            ...
            for obj in &road.objects {
                if obj.object_type == ObjectType::ParkingSpace {
                    ...
                    assert!(
                        ...
                        obj.id, road.id, s, road.length
                    );
                    println!(
                        "  Parking {}: s={:.2} t={:.2} hdg={:.4} corners={}",
                        obj.id, s, obj.position.y, obj.hdg, obj.corners.len()
                    );
                }
            }
        }
