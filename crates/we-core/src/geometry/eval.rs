...
fn fresnel_cs(t: f64) -> (f64, f64) {
    ...
    if t < 1.5 {
        ...
        let pf = yp * (1.3564119068e1 + yp * (2.0153207340e2 + yp * (1.2079076192e3
            + yp * (3.5048398426e3 + yp * 5.3583606480e3))));
        let qf = 1.0 + y * (1.8746587340e1 + yp * (2.9208480030e2 + yp * (1.8816558750e3
            + yp * (5.4356601570e3 + yp * 5.8924130890e3))));

        let pg = yp * (1.4173782370e1 + yp * (2.1287938700e2 + yp * (1.2751872650e3
            + yp * (3.6750998840e3 + yp * 5.5828251430e3))));
        let qg = 1.0 + y * (1.9299692760e1 + yp * (3.0107256000e2 + yp * (1.9419772020e3
            + yp * (5.6168010720e3 + yp * 6.1163645470e3))));
...
#[cfg(test)]
fn evaluate_spiral_simpson(curv_start: f64, curv_end: f64, length: f64, ds: f64) -> (f64, f64, f64) {
    if length < 1e-15 {
        return (0.0, 0.0, 0.0);
    }
    let c_dot = (curv_end - curv_start) / length;
...
mod tests {
        ...
        for (cs, ce, len) in &cases {
            for ds_frac in &[0.25, 0.5, 0.75, 1.0] {
                ...
                assert!(
                    ...
                    cs, ce, len, ds, opt_x, opt_y, sim_x, sim_y, err_x, err_y
                );
            }
        }
    }
    ...
    fn test_fresnel_known_values() {
        ...
        assert!((c - 0.779893).abs() < 1e-5, "C(1) = {}, expected ~0.779893", c);
        assert!((s - 0.438259).abs() < 1e-5, "S(1) = {}, expected ~0.438259", s);
